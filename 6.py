...
class RequestSenderApp:
    def __init__(self, root):
        self.root = root
        self.root.title("HTTP Request Sender - 4 Sections")
        ...
        # Title
        tk.Label(main_container, text="HTTP Request Sender",
                font=('Arial', 16, 'bold')).pack(pady=(0, 10))

        # Global Control Panel
        control_frame = tk.Frame(main_container, bg='#E0E0E0', padx=15, pady=10)
        control_frame.pack(fill='x', pady=(0, 10))

        tk.Label(control_frame, text="Global Control:", font=('Arial', 11, 'bold'),
                bg='#E0E0E0').pack(side='left', padx=(0, 15))

        # Global Price input
        tk.Label(control_frame, text="Price:", font=('Arial', 10),
                bg='#E0E0E0').pack(side='left', padx=(0, 5))
        self.global_price_var = tk.StringVar()
        self.global_price_entry = tk.Entry(control_frame, textvariable=self.global_price_var, width=8,
                                           font=('Arial', 10))
        self.global_price_entry.pack(side='left', padx=(0, 10))

        # Global Size input
        tk.Label(control_frame, text="Size:", font=('Arial', 10),
                bg='#E0E0E0').pack(side='left', padx=(0, 5))
        self.global_size_var = tk.StringVar()
        self.global_size_entry = tk.Entry(control_frame, textvariable=self.global_size_var, width=8,
                                          font=('Arial', 10))
        self.global_size_entry.pack(side='left', padx=(0, 10))
        ...
        self.apply_global_button = tk.Button(control_frame, text="Apply All",
                                             ...
                                             font=('Arial', 10, 'bold'),
                                             width=8)
        self.apply_global_button.pack(side='left', padx=(0, 20))

        # Delay input
        tk.Label(control_frame, text="Delay (ms):", font=('Arial', 10),
                bg='#E0E0E0').pack(side='left', padx=(0, 5))
        self.delay_var = tk.StringVar(value="500")
        self.delay_entry = tk.Entry(control_frame, textvariable=self.delay_var, width=8,
                                    font=('Arial', 10))
        ...
        self.global_status = tk.Label(control_frame, text="Ready", font=('Arial', 10),
                                      bg='#E0E0E0', fg='gray')
        self.global_status.pack(side='left', padx=10)
        
        # Create horizontal container for 4 sections
        sections_container = tk.Frame(main_container)
        sections_container.pack(fill='both', expand=True, pady=(0, 10))
        
        # Create 4 sections side by side
        self.sections = []
        for i in range(4):
            section = self.create_section(sections_container, i+1)
            section.pack(side='left', fill='both', expand=True, padx=5)
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, height=12, width=140)
        self.log_text.pack(fill='both', expand=True)
        
        # Clear Log Button
        tk.Button(log_frame, text="Clear Log", command=self.clear_log,
                 bg='#FF9800', fg='white', font=('Arial', 10)).pack(anchor='e', pady=5)
    
    def create_section(self, parent, section_num):
        """Create a section for one request"""
        ...
        request_text = scrolledtext.ScrolledText(section_frame, height=6, width=35)
        request_text.pack(fill='both', expand=True, padx=5, pady=5)
        ...
        # Price
        tk.Label(params_frame, text="Price:", width=8, anchor='w').pack(anchor='w', pady=2)
        price_var = tk.StringVar()
        price_entry = tk.Entry(params_frame, textvariable=price_var, width=25)
        price_entry.pack(fill='x', pady=(0, 5))
        
        # Size
        tk.Label(params_frame, text="Size:", width=8, anchor='w').pack(anchor='w', pady=2)
        size_var = tk.StringVar()
        size_entry = tk.Entry(params_frame, textvariable=size_var, width=25)
        size_entry.pack(fill='x', pady=(0, 5))
        
        # Side (Radio Buttons)
        tk.Label(params_frame, text="Side:", width=8, anchor='w').pack(anchor='w', pady=2)
        # Section 1,3 = BACK, Section 2,4 = LAY
        default_side = "BACK" if section_num % 2 == 1 else "LAY"
        side_var = tk.StringVar(value=default_side)
        
        side_frame = tk.Frame(params_frame)
        side_frame.pack(fill='x', pady=(0, 5))
        
        lay_radio = tk.Radiobutton(side_frame, text="LAY", variable=side_var, value="LAY")
        lay_radio.pack(side='left', padx=(0, 10))
        
        back_radio = tk.Radiobutton(side_frame, text="BACK", variable=side_var, value="BACK")
        back_radio.pack(side='left')
        
        # Bet UUID
        tk.Label(params_frame, text="UUID:", width=8, anchor='w').pack(anchor='w', pady=2)
        uuid_var = tk.StringVar()
        uuid_entry = tk.Entry(params_frame, textvariable=uuid_var, width=25)
        uuid_entry.pack(fill='x', pady=(0, 10))
        
        # Send Button
        send_button = tk.Button(section_frame, text=f"Send Section {section_num}",
        ...
        status_label = tk.Label(section_frame, text="Ready", fg='gray', font=('Arial', 9))
        ...
        try:
            ...
            if isinstance(json_data, dict):
                ...
                if isinstance(json_data[market_key], list) and len(json_data[market_key]) > 0:
                    ...
                    # Update the parameter fields (but NOT the side - that stays as section default)
                    section_data['price_var'].set(str(bet_data.get('price', '')))
                    section_data['size_var'].set(str(bet_data.get('size', '')))
                    # Don't update side_var - keep section default (Section 1,3=BACK, Section 2,4=LAY)
                    section_data['uuid_var'].set(str(bet_data.get('betUuid', '')))
                    
                    section_data['status_label'].config(text=f"Loaded successfully!", fg='green')
                    
                    # Log to common log
                    self.log(f"Section {section_num}: Request loaded - Price: {bet_data.get('price')}, Size: {bet_data.get('size')}, Market: {market_key}")
                else:
    ...
    def send_request(self, section_num):
        ...
        try:
            ...
            # Log sending
            self.log(f"Section {section_num}: Sending request to {section_data['url']}")
            self.log(f"Section {section_num}: Price={price}, Size={size}, Side={side}")
    ...
    def _handle_response(self, result):
        ...
        if response.status_code == 200:
            try:
                ...
                if has_error:
                    section_data['status_label'].config(text="FAILED - See Log", fg='red')
                ...
                section_data['status_label'].config(text="Success (non-JSON)", fg='green')
